...
-- ======================================================
-- القسم 3: نموذج خطوط الفاتورة (ديناميكي)
-- ======================================================
...
-- ======================================================
-- القسم 4: فترات الفوترة ورفع القراءات
-- ======================================================
...
-- 4.3 القراءات النهائية (المفوترة)
...
CREATE TABLE payment (
    ...
    source_type ENUM('cash', 'bank_transfer', 'card', 'online', 'cheque', 'collector', 'ewallet') NOT NULL DEFAULT 'cash',
    collector_id INT NULL,
    ewallet_trans_id INT NULL,
...
-- ======================================================
-- القسم 6: الرصيد والتسويات المالية
-- ======================================================
...
-- ======================================================
-- القسم 7: نظام معالجة الفواتير (Queue)
-- ======================================================

-- 7.1 طابور معالجة الفواتير
...
-- ======================================================
-- القسم 8: نظام إدارة المتحصلين والصناديق المالية
-- ======================================================
...
-- 8.2 الصناديق المالية للمتحصلين
...
-- ======================================================
-- القسم 9: نظام المحافظ الإلكترونية
-- ======================================================

-- 9.1 أنواع المحافظ الإلكترونية
...
-- ======================================================
-- القسم 10: نظام الكاشف (Meter Reader)
-- ======================================================
...
-- ======================================================
-- القسم 11: نظام المستخدمين والأدوار
-- ======================================================
...
-- 11.6 ربط المستخدم بالكاشف والمتحصل
...
-- ======================================================
-- القسم 12: نظام خطوط السير (Routes) المبسط
-- ======================================================

-- 12.1 خط السير الرئيسي
CREATE TABLE route (
...
-- 12.2 عقود الخط (تم استبدال route_meter بـ route_contract)
...
-- ======================================================
-- القسم 15: الفهارس المحسنة (Indexes)
-- ======================================================
...
-- ======================================================
-- القسم 16: البيانات الافتراضية (Seed Data)
-- ======================================================
...
-- 16.2 قوالب خطوط الفاتورة للسكني
...
-- 16.3 تفاصيل شرائح الاستهلاك للسكني
...
('PaymentReceived', 'تأكيد دفع', 'شكراً {customer_name}، تم استلام مبلغ {amount} دينار عن الفاتورة {invoice_number}. رصيدك المتبقي: {remaining_balance} دينار', 'customer_name,amount,invoice_number,remaining_balance'),
...
-- 16.9 صلاحيات المدير (جميع الصلاحيات)
INSERT INTO role_permission (role_id, permission_id)
SELECT 1, permission_id FROM permission;

-- 16.10 إعدادات النظام
...
('sms_auto_send', '1', 'تفعيل الإرسال التلقائي للرسائل'),
...
-- 16.12 مستخدم افتراضي (admin/admin123)
INSERT INTO system_user (username, password_hash, full_name_ar, mobile_phone, national_id, user_status) VALUES
('admin', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'مدير النظام', '0500000000', '0000000000', 'active');

-- إضافة دور المدير للمستخدم admin
INSERT INTO user_role (user_id, role_id, assigned_by)
VALUES (1, 1, 1);

-- ======================================================
-- القسم 17: الإجراءات المخزنة (Stored Procedures)
-- ======================================================

DELIMITER $$

-- 17.1 تحديث رصيد العميل
...
BEGIN
    ...
    INSERT INTO payment (
        ...
        payment_method, source_type, collector_id, reference_number, notes
    ) VALUES (
        v_payment_number, v_invoice_id, v_customer_id, p_amount, NOW(),
        p_payment_method, 'collector', v_collector_id, p_reference_number, p_notes
    );
...
BEGIN
    ...
    IF p_execution_type = 'reading' THEN
        SELECT meter_number, meter_id INTO v_meter_number, v_meter_id
        FROM meter
...
-- 17.11 تقرير تفاصيل جولة محددة
CREATE PROCEDURE GetRouteDetailsReport(IN p_assignment_id INT)
BEGIN
    SELECT 
        ra.assignment_id,
        r.route_code,
        ...
        r.route_type,
        CASE r.route_type
            WHEN 'meter_reading' THEN 'قراءات فقط'
            WHEN 'collection' THEN 'تحصيل فقط'
            WHEN 'both' THEN 'قراءات وتحصيل'
        END AS route_type_ar,
        CASE ra.assigned_to_type
            WHEN 'user' THEN (SELECT full_name_ar FROM system_user WHERE user_id = ra.assigned_to_id)
            WHEN 'meter_reader' THEN (SELECT full_name_ar FROM meter_reader WHERE meter_reader_id = ra.assigned_to_id)
            WHEN 'collector' THEN (SELECT full_name_ar FROM collector WHERE collector_id = ra.assigned_to_id)
        ...
        ra.started_at,
        ra.completed_at,
        TIMESTAMPDIFF(MINUTE, ra.started_at, ra.completed_at) AS duration_minutes
    ...
    SELECT 
        re.stop_order,
        c.contract_number,
        cust.full_name_ar AS customer_name,
        cust.mobile_phone,
        m.meter_number,
        re.execution_status,
        CASE re.execution_status
            WHEN 'pending' THEN 'في انتظار التنفيذ'
            WHEN 'done' THEN 'تم التنفيذ'
            WHEN 'skipped' THEN 'تم التخطي'
        END AS status_ar,
        re.actual_reading,
        re.actual_amount,
        re.skip_reason,
    ...
    FROM route_execution re
    JOIN route_contract rc ON re.route_contract_id = rc.route_contract_id
    JOIN contract c ON rc.contract_id = c.contract_id
    JOIN customer cust ON c.customer_id = cust.customer_id
    LEFT JOIN meter m ON c.contract_id = m.contract_id AND m.meter_status = 'active'
    WHERE re.assignment_id = p_assignment_id
    ORDER BY re.stop_order;
    
    SELECT 
        COUNT(*) AS total_contracts,
        SUM(CASE WHEN execution_status = 'done' THEN 1 ELSE 0 END) AS completed_contracts,
        SUM(CASE WHEN execution_status = 'pending' THEN 1 ELSE 0 END) AS pending_contracts,
        SUM(CASE WHEN execution_status = 'skipped' THEN 1 ELSE 0 END) AS skipped_contracts,
        SUM(actual_amount) AS total_collected,
        COUNT(CASE WHEN actual_reading IS NOT NULL THEN 1 END) AS readings_done
    FROM route_execution re
    WHERE re.assignment_id = p_assignment_id;
    
END$$

-- 17.12 تقرير المدفوعات حسب المصدر
CREATE PROCEDURE GetPaymentSourcesReport(
    IN p_start_date DATE,
    IN p_end_date DATE
)
BEGIN
    SELECT 
        ...
        CASE source_type
            WHEN 'cash' THEN 'نقدي'
            WHEN 'collector' THEN 'تحصيل ميداني'
            WHEN 'ewallet' THEN 'محفظة إلكترونية'
            WHEN 'bank_transfer' THEN 'تحويل بنكي'
            WHEN 'card' THEN 'بطاقة ائتمان'
...
-- ======================================================
-- القسم 18: الأحداث المجدولة (Events)
-- ======================================================

SET GLOBAL event_scheduler = ON;

-- حدث: معالجة طابور الفوترة كل 5 دقائق
CREATE EVENT IF NOT EXISTS process_billing_queue_event
ON SCHEDULE EVERY 5 MINUTE
DO
BEGIN
    DECLARE v_batch_size INT;
    SELECT setting_value INTO v_batch_size FROM system_settings WHERE setting_key = 'billing_queue_batch_size';
    SET @batch_size = COALESCE(v_batch_size, 50);
    
    -- معالجة طابور الفوترة (يمكن تفعيلها لاحقاً حسب الحاجة)
    -- CALL ProcessBillingQueueBatch(@batch_size);
END$$

-- حدث: إغلاق فترات القراءة تلقائياً
CREATE EVENT IF NOT EXISTS auto_close_reading_periods
ON SCHEDULE EVERY 1 DAY
DO
BEGIN
    UPDATE billing_period
    SET status = 'reading_closed'
    WHERE status = 'reading_open'
      AND reading_end_date < CURDATE();
END$$
