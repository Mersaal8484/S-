...
_logger = logging.getLogger('Contract corn jop')
...
class AccountAnalyticAccount(models.Model):
    ...
    recurring_invoice_meterline_ids = fields.One2many(
        'account.analytic.meter.line', 'a_meterline_id', string='Meter Lines', copy=False)
    recurring_invoice_meterlineinv_ids = fields.One2many(
        'account.analytic.meter.line.inv', 'a_meterline_id', string='Meter Lines Inv', copy=False)
    ...
    @api.one
    @api.constrains('meter_contractid')
    def _check_unique_constraint_meter_contractid(self):
        if self.recurring_invoices:
            if len(self.search([('meter_contractid', '=', self.meter_contractid)])) > 1:
                raise ValidationError("رقم العقد موجود مسبقاً....")


    @api.one
    @api.constrains('meter_id')
    def _check_unique_constraint_meter_id(self):
        if self.recurring_invoices:
            if len(self.search([('meter_id', '=', self.meter_id)])) > 1:
                raise ValidationError("رقم العداد موجود مسبقاً...")
    ...
    @api.multi
    def _convert_contract_meter_lines(self, contract):
        self.ensure_one()
        new_lines = []
        for contract_line in contract.recurring_invoice_meterline_ids:
            vals = contract_line._convert_to_write(contract_line.read()[0])
            new_lines.append((0, 0, vals))
        return new_lines
    ...
    @api.model
    def get_quantity_subscription(self):
        for line in self:
            ...
            if last_id:
                start_date = datetime.strptime(last_id.date_readed, '%Y-%m-%d %H:%M:%S').date()
            else:
                start_date = datetime.strptime(line.date_Sub_start, '%Y-%m-%d').date()
            
            end_date = datetime.strptime(line.recurring_next_date, '%Y-%m-%d').date()
            delta = end_date - start_date
            return 15
        
    @api.model
    def get_quantity_start_invoce(self):
        for line in self:
            last_id = self.env['account.analytic.meter.line.inv'].search([('a_meterline_id','=',line.id),('isinvoced','=',True)], order='id desc', limit=1)
            if last_id:
                return 0
            else:
                return 1
    ...
    def get_quantity_subscription1(self):
        for line in self:
            ...
            if delta.days-1 <= 10:
                return "1"
            elif delta.days-1 <= 20:
                return "1,2"
            elif delta.days-1 <= 30:
                 return "1,2,3"
            elif delta.days-1 <= 40:
                return "1,2,3,4"
            else:
                return "1,2,3,4,5"
    ...
    def _prepare_invoice(self):
        ...
        invoice = self.env['account.invoice'].new({
            ...
            #'invmeter_current_reading': self.meter_current_reading,
            'invmeter_last_reading': self.meter_lastInvo_reading,
            'invdatetime_lastreading': self.datetime_lastreading,
        })
    ...
    def _create_invoice(self):
        ...
        for line_reading in self.recurring_invoice_meterline_ids:
            if line_reading.image:
                #invoice_vals['inv_meter_image'] = line_reading.image
                invoice_vals['inv_meter_image2'] = line_reading.image
                break
        invoice = self.env['account.invoice'].create(invoice_vals)
        for line in self.recurring_invoice_line_ids:
            invoice_line_vals = self._prepare_invoice_line(line, invoice.id)
            #if invoice_line_vals['product_id'] == 12 and invoice_line_vals['quantity'] <= 0:
            #    continue
            self.env['account.invoice.line'].create(invoice_line_vals)
        invoice.compute_taxes()
        self.update_meter_reading(invoice.id)
        self.recurring_next_date = self.env.context['next_date'].strftime('%Y-%m-%d')
        return invoice
    
    @api.multi
    def update_meter_reading(self,invoice_id):
        """ Get the date of the next occurrence """
        isinvoce = True
        name =  self.name + ' - ' +self.meter_id
        my_date = datetime.strptime(self.recurring_next_date, '%Y-%m-%d').date()
        my_time = datetime.min.time()
        my_datetime = datetime.combine( my_date, my_time)
        d = my_datetime
        meter_line_vals={'a_meterline_id':self.id,
                         'name':name,
                         'mseterLine_current_reading':self.meter_current_reading,
                         'mseterLine_old_reading':self.meter_lastInvo_reading,
                         'date_readed':d,
                         'isinvoced': isinvoce,
                         'totalmeter_usedunit': self.meter_usedunit,
                         'inv_refrid': invoice_id,}
        #for line_reading in self.recurring_invoice_meterline_ids:
            #if line_reading.image:
               #meter_line_vals['image'] = line_reading.image
                #break
        self.env['account.analytic.meter.line.inv'].create(meter_line_vals)
        self.recurring_invoice_meterline_ids.unlink()
        #for line in self.recurring_invoice_meterline_ids:
        #    line.unlink()
            #line.update({
            #'isinvoced': isinvoce,
            #'totalmeter_usedunit': self.meter_usedunit,
    ...
    def check_dates_valid(self):
        ...
        if self.recurring_next_date > fields.Date.today():
            return False
    ...
    def get_ctx(self):
        ...
        new_date = old_date + self.get_relative_delta(self.recurring_rule_type, self.recurring_interval)
        ctx = self.env.context.copy()
        ctx.update({
            'old_date': old_date,
            'next_date': new_date,
            # add prevent mail notification keys
    ...
    def recurring_create_invoice(self):
        ...
        for contract in self:
            ...
            try:
                #ref_date = self.recurring_next_date or fields.Date.today()
                _logger.info('Contract Invoice: %s', contract.name) 
                #if contract.recurring_next_date > fields.Date.today():
                #    continue
                #if (contract.date_start > ref_date or contract.date_end and contract.date_end < ref_date):
                #    if self.env.context.get('cron'):
                ...
                #ref_date = self.recurring_next_date or fields.Date.today()
                #old_date = fields.Date.from_string(ref_date)
                #new_date = old_date + self.get_relative_delta(contract.recurring_rule_type, contract.recurring_interval)
                ctx = contract.get_ctx()
                #ctx = self.env.context.copy()
                #ctx.update({
                #'old_date': old_date,
                #'next_date': new_date,
                ...
                # continue
                _logger.exception('Error during Contract invoice order creation: %s', contract.name)    
        return invoices

    @api.model
    def cron_recurring_create_invoice(self,arg1=300,arg2=702):
        today = fields.Date.today()
        unetused=self.meter_canusedunit+arg1
        contracts = self.with_context(cron=True).search([
            ('recurring_invoices', '=', True),
            ('recurring_next_date', '<=', today),
            ('meter_isinv_ready', '=', True),
            ('meter_usedunit', '<', unetused),
            ('meter_usedunit', '>=', 0),
            ('partner_id.nuts1_id', '=', arg2),
            '|',
            ('date_end', '=', False),
            ('date_end', '>=', today),
        ], limit=60)
    ...
    @api.model
    def cron_recurring_create_invoice1(self,arg1=300,arg2=702,arg3=60):
        today = fields.Date.today()
        unetused=self.meter_canusedunit+arg1
        contracts = self.with_context(cron=True).search([
            ('recurring_invoices', '=', True),
    ...
    @api.model
    def cron_recurring_create_invoice2(self,arg1=300,arg2=702,arg3=60):
        today = fields.Date.today()
        unetused=self.meter_canusedunit+arg1
        contracts = self.with_context(cron=True).search([
            ('recurring_invoices', '=', True),
